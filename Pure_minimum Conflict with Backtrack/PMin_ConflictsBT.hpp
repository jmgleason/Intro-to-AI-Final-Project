...
QueenGrid Min_Conflicts( struct QueenGrid QG, int MaxSteps ){
    ...
    for( int step=0; step<MAX_STEPS; ++step ){
        ...
        struct Queen newPosition = findNewPosition( selectedQueen, QG.Grid, QG.AvoidMap );
       ...
       QG.FrequencyMap[newPosition.pos.x][newPosition.pos.y]++;
        ...
        //Update avoidMap every numOfQueen*3 steps
        if( frequencyCount > numOfQueens*3 ){
            for( int i=0; i<numOfQueens; ++i ){
                if( QG.ConflictQueens[i].active == true ){
                    if( QG.FrequencyMap[QG.ConflictQueens[i].pos.x][QG.ConflictQueens[i].pos.y] > numOfQueens && QG.AvoidMap[QG.ConflictQueens[i].pos.x][QG.ConflictQueens[i].pos.y] == false ){
                        QG.AvoidMap[QG.ConflictQueens[i].pos.x][QG.ConflictQueens[i].pos.y] = true;
                    }
                }
            }
            frequencyCount = 0;
    ...
    std::cout<<"FAIL : It reached maximum iteration steps of "<<MAX_STEPS<<std::endl;
    //Return failure
    QG.fail = true;
    return QG;
};
